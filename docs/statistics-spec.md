...
Assumption honesty: the symmetry null is stronger than "E[d] = 0". A
distribution with mean exactly 0 but asymmetric (rare in this design, since
under a true null of exchangeable variants symmetry holds by construction)
is outside the null. The report states the null as "no difference between
variants," which is the exchangeability null — that is what is actually
being tested.
...
one-sided tests at α = 0.05); if the question is "is the candidate not worse
